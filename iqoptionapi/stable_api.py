...
class IQOptionUser:
    """
    Classe para gerenciar informações de usuários e operações auxiliares na IQ Option.

    Args:
        api (Any): Instância da API da IQ Option.
    """

    def __init__(self, api: Any) -> None:
        self.api = api
        self.timeout = 30  # Tempo limite padrão em segundos.

    def get_user_profile_client(self, user_id: int) -> Optional[Dict]:
        """
        ...
        Returns:
            Optional[Dict]: Dados do perfil do usuário ou None em caso de erro.
        """
        self.api.user_profile_client = None
        self.api.Get_User_Profile_Client(user_id)
        ...
        while self.api.user_profile_client is None:
            if time.time() - start_time > self.timeout:
                logging.error(f"Timeout ao obter o perfil do usuário {user_id}.")
                return None

        return self.api.user_profile_client

    def request_leaderboard_userinfo_deals_client(self, user_id: int, country_id: int) -> Optional[Dict]:
        """
        Solicita informações do usuário no leaderboard.
        ...
        Returns:
            Optional[Dict]: Dados do leaderboard ou None em caso de erro.
        """
        self.api.leaderboard_userinfo_deals_client = None
        ...
        while True:
            try:
                if self.api.leaderboard_userinfo_deals_client.get("isSuccessful", False):
                    return self.api.leaderboard_userinfo_deals_client
            ...
            if time.time() - start_time > self.timeout:
                logging.error(f"Timeout ao obter informações do leaderboard para o usuário {user_id}.")
                return None

    def get_users_availability(self, user_id: int) -> Optional[Dict]:
        """
        ...
        Returns:
            Optional[Dict]: Dados de disponibilidade do usuário ou None em caso de erro.
        """
        self.api.users_availability = None
        start_time = time.time()
        ...
        while self.api.users_availability is None:
            ...
            if time.time() - start_time > self.timeout:
                logging.error(f"Timeout ao verificar disponibilidade do usuário {user_id}.")
                return None
    ...
    def logout(self) -> None:
        """
        Desconecta o usuário da IQ Option.
        """
        self.api.logout()
        logging.info("Logout realizado com sucesso.")
        
class IQOptionDigital:
    """
    Classe para gerenciar operações digitais na IQ Option.
    ...
    def get_digital_current_profit(self, active: str, duration: int) -> Optional[float]:
        """
        Obtém o lucro atual para um ativo digital com base na duração.

        Args:
            active (str): Nome do ativo.
            duration (int): Duração em minutos.

        Returns:
            Optional[float]: Lucro atual (se encontrado) ou None.
        """
        try:
            profit_data = self.api.instrument_quites_generated_data[active][duration * 60]
            for key, value in profit_data.items():
                if "SPT" in key:
                    return value
        except KeyError:
            logging.error(f"Erro ao obter lucro para {active} com duração {duration} minutos.")
        return None

    def buy_digital_spot(self, active: str, amount: float, action: str, duration: int) -> Tuple[bool, Optional[int]]:
        """
        Realiza a compra de uma opção digital com base no ativo, quantidade, direção e duração.

        Args:
            active (str): Nome do ativo.
            amount (float): Valor da compra.
            action (str): Direção da opção ('put' ou 'call').
            duration (int): Duração em minutos.

        Returns:
            Tuple[bool, Optional[int]]: Sucesso da operação e ID da ordem, se bem-sucedida.
        """
        # Formatação do tipo de ação
        action = action.lower()
        action_code = {"put": "P", "call": "C"}.get(action)
        if not action_code:
            logging.error(f"Ação inválida '{action}' para buy_digital_spot.")
            return False, None
        ...
        if duration == 1:
            ...
            while True:
                if now_date.minute % duration == 0 and (now_date.timestamp() - timestamp) > 30:
                    break
                now_date += timedelta(minutes=1)
            exp = int(now_date.timestamp())

        # Formata o ID do instrumento
        ...
        while time.time() - start_time < self.timeout:
            order_id = self.api.digital_option_placed_id.get(request_id)
            if order_id is not None:
                if isinstance(order_id, int):
                    return True, order_id
                return False, order_id

        logging.error(f"Timeout ao tentar realizar a compra digital para {active}.")
        return False, None

    def get_digital_payout(self, active: str, seconds: int = 0) -> float:
        """
        Obtém o payout digital para um ativo.

        Args:
            active (str): Nome do ativo.
            seconds (int, opcional): Tempo máximo para aguardar o payout.

        Returns:
            float: Valor do payout ou 0 em caso de erro.
        """
        self.api.digital_payout = None
        asset_id = OP_code.ACTIVES[active]
        self.api.subscribe_digital_price_splitter(asset_id)
        ...
        start_time = time.time()
        while self.api.digital_payout is None:
            if seconds and (time.time() - start_time) > seconds:
                break

        self.api.unsubscribe_digital_price_splitter(asset_id)
        return self.api.digital_payout or 0.0

    def close_digital_option(self, position_id: int) -> Optional[Dict]:
        """
        Fecha uma posição digital com base no ID da posição.

        Args:
            position_id (int): ID da posição a ser fechada.

        Returns:
            Optional[Dict]: Resposta do fechamento ou None em caso de erro.
        """
        start_time = time.time()

        # Aguarda até que a posição esteja disponível
        while time.time() - start_time < self.timeout:
            async_data = self.get_async_order(position_id)
            if async_data and async_data["position-changed"]:
                position_changed = async_data["position-changed"]["msg"]
                self.api.close_digital_option(position_changed["external_id"])
                while self.api.result is None:
                    pass
                return self.api.result

        logging.error(f"Timeout ao tentar fechar a posição {position_id}.")
        return None

    def get_async_order(self, buy_order_id: int) -> Dict:
        """
        Obtém dados assíncronos de uma ordem específica.
...
class IQOptionOrders:
    """
    Classe para gerenciar pedidos, posições e alavancagens na IQ Option.
    ...
    def buy_order(self, 
                  ...
                  order_type: str, limit_price: Optional[float] = None, stop_price: Optional[float] = None, 
                  stop_loss_kind: Optional[str] = None, stop_loss_value: Optional[float] = None, 
                  ...
                  use_token_for_commission: bool = False) -> Tuple[bool, Optional[int]]:
        """
        Realiza uma ordem de compra para Forex, CFDs, ou outros instrumentos.

        Args:
            instrument_type (str): Tipo de instrumento (e.g., 'forex', 'cfd').
            instrument_id (str): ID do instrumento.
            side (str): Lado da compra ('buy' ou 'sell').
            amount (float): Quantidade a ser comprada.
            leverage (int): Alavancagem.
            order_type (str): Tipo de ordem ('market', 'limit').
            limit_price (Optional[float]): Preço limite (se aplicável).
            stop_price (Optional[float]): Preço de stop (se aplicável).
            stop_loss_kind (Optional[str]): Tipo de stop loss (percentual ou valor fixo).
            stop_loss_value (Optional[float]): Valor do stop loss.
            take_profit_kind (Optional[str]): Tipo de take profit.
            take_profit_value (Optional[float]): Valor do take profit.
            use_trail_stop (bool): Se deve usar trailing stop.
            auto_margin_call (bool): Se deve ativar auto margin call.
            use_token_for_commission (bool): Se deve usar token para reduzir comissões.

        Returns:
            Tuple[bool, Optional[int]]: Sucesso da operação e ID da ordem.
        """
        self.api.buy_order_id = None
        self.api.buy_order(
            instrument_type=instrument_type, instrument_id=instrument_id,
            side=side, amount=amount, leverage=leverage, type=order_type,
            limit_price=limit_price, stop_price=stop_price,
            stop_lose_kind=stop_loss_kind, stop_lose_value=stop_loss_value,
            take_profit_kind=take_profit_kind, take_profit_value=take_profit_value,
            use_trail_stop=use_trail_stop, auto_margin_call=auto_margin_call,
            use_token_for_commission=use_token_for_commission
        )
        ...
        while self.api.buy_order_id is None:
            if time.time() - start_time > self.timeout:
                logging.error("Timeout ao realizar a ordem de compra.")
                return False, None

        check, data = self.get_order(self.api.buy_order_id)
        if check and data.get("status") != "rejected":
            return True, self.api.buy_order_id
        else:
            return False, data.get("reject_status", None)

    def cancel_order(self, buy_order_id: int) -> bool:
        """
        Cancela uma ordem existente pelo ID.

        Args:
            buy_order_id (int): ID da ordem.

        Returns:
            bool: True se a ordem foi cancelada, False caso contrário.
        """
        self.api.order_canceled = None
        self.api.cancel_order(buy_order_id)
        ...
        while self.api.order_canceled is None:
            if time.time() - start_time > self.timeout:
                logging.error(f"Timeout ao cancelar a ordem {buy_order_id}.")
                return False
    ...
    def close_position(self, position_id: int) -> bool:
        """
        Fecha uma posição existente pelo ID.

        Args:
            position_id (int): ID da posição.

        Returns:
            bool: True se a posição foi fechada, False caso contrário.
        """
        check, data = self.get_order(position_id)
        if data and data.get("position_id"):
            ...
            while self.api.close_position_data is None:
                if time.time() - start_time > self.timeout:
                    logging.error(f"Timeout ao fechar a posição {position_id}.")
                    return False

            return self.api.close_position_data.get("status") == 2000
        else:
            logging.error(f"Posição {position_id} não encontrada.")
            return False

    def get_order(self, buy_order_id: int) -> Tuple[bool, Optional[Dict]]:
        """
        Obtém informações detalhadas sobre uma ordem pelo ID.

        Args:
            buy_order_id (int): ID da ordem.

        Returns:
            Tuple[bool, Optional[Dict]]: Sucesso e detalhes da ordem.
        """
        self.api.order_data = None
        self.api.get_order(buy_order_id)
        ...
        while self.api.order_data is None:
            if time.time() - start_time > self.timeout:
                logging.error(f"Timeout ao obter dados da ordem {buy_order_id}.")
                return False, None
    ...
    def get_position_history(self, instrument_type: str) -> Tuple[bool, Optional[Dict]]:
        """
        Obtém o histórico de posições para um tipo de instrumento.

        Args:
            instrument_type (str): Tipo de instrumento (e.g., 'forex', 'cfd').

        Returns:
            Tuple[bool, Optional[Dict]]: Sucesso e histórico das posições.
        """
        self.api.position_history = None
        self.api.get_position_history(instrument_type)
        ...
        while self.api.position_history is None:
            if time.time() - start_time > self.timeout:
                logging.error(f"Timeout ao obter histórico de posições para {instrument_type}.")
                return False, None
    ...
    def get_available_leverages(self, instrument_type: str, active: Optional[str] = "") -> Tuple[bool, Optional[Dict]]:
        """
        Obtém as alavancagens disponíveis para um tipo de instrumento.

        Args:
            instrument_type (str): Tipo de instrumento (e.g., 'forex').
            active (Optional[str]): Nome do ativo (opcional).

        Returns:
            Tuple[bool, Optional[Dict]]: Sucesso e alavancagens disponíveis.
        """
        self.api.available_leverages = None
        if active:
            self.api.get_available_leverages(instrument_type, OP_code.ACTIVES[active])
        else:
            self.api.get_available_leverages(instrument_type, "")
        ...
        while self.api.available_leverages is None:
            if time.time() - start_time > self.timeout:
                logging.error(f"Timeout ao obter alavancagens para {instrument_type}.")
                return False, None
...
class IQOptionHelpers:
    """
    Classe para métodos auxiliares e interações específicas com a API.
    ...
    def subscribe_live_deal(self, name: str, active: str, deal_type: str, buffer_size: int) -> None:
        """
        Assina live deals para um ativo específico.

        Args:
            name (str): Nome do live deal.
            active (str): Nome do ativo.
            deal_type (str): Tipo do live deal.
            buffer_size (int): Tamanho do buffer para armazenar dados.
        """
        active_id = OP_code.ACTIVES[active]
        self.api.Subscribe_Live_Deal(name, active_id, deal_type)

        # Inicializa o buffer para armazenar os dados
        self.api.live_deal_data[name][active][deal_type] = deque([], buffer_size)

    def unsubscribe_live_deal(self, name: str, active: str, deal_type: str) -> None:
        """
        Cancela a assinatura de live deals para um ativo específico.
        ...
        """
        active_id = OP_code.ACTIVES[active]
        self.api.Unscribe_Live_Deal(name, active_id, deal_type)
        self.api.live_deal_data[name][active].pop(deal_type, None)

    def clear_live_deal(self, name: str, active: str, deal_type: str, buffer_size: int) -> None:
        """
        Limpa os dados de live deals e redefine o buffer.

        Args:
            name (str): Nome do live deal.
            active (str): Nome do ativo.
            deal_type (str): Tipo do live deal.
            buffer_size (int): Novo tamanho do buffer.
        """
        self.api.live_deal_data[name][active][deal_type] = deque([], buffer_size)

    def get_live_deal(self, name: str, active: str, deal_type: str) -> List:
        """
        Obtém os dados armazenados de live deals para um ativo específico.
        ...
        Returns:
            List: Dados armazenados do live deal.
        """
        return list(self.api.live_deal_data[name][active][deal_type])

    def pop_live_deal(self, name: str, active: str, deal_type: str) -> Optional[Dict]:
        """
        Remove o item mais antigo do live deal armazenado.
        ...
        Returns:
            Optional[Dict]: Dados do live deal removido ou None se vazio.
        """
        try:
            return self.api.live_deal_data[name][active][deal_type].pop()
        except IndexError:
            logging.warning(f"Nenhum dado encontrado para {name}, {active}, {deal_type}.")
            return None

    def get_user_profile_client(self, user_id: int) -> Optional[Dict]:
        """
        Obtém o perfil do cliente pelo ID do usuário.

        Args:
            user_id (int): ID do usuário.

        Returns:
            Optional[Dict]: Dados do perfil do cliente ou None em caso de erro.
        """
        self.api.user_profile_client = None
        self.api.Get_User_Profile_Client(user_id)
        ...
        while self.api.user_profile_client is None:
            if time.time() - start_time > self.timeout:
                logging.error(f"Timeout ao obter o perfil do cliente {user_id}.")
                return None

        return self.api.user_profile_client

    def opcode_to_name(self, opcode: int) -> Optional[str]:
        """
        Converte um opcode para o nome do ativo.

        Args:
            opcode (int): Opcode do ativo.

        Returns:
            Optional[str]: Nome do ativo ou None se não encontrado.
        """
        try:
            return list(OP_code.ACTIVES.keys())[list(OP_code.ACTIVES.values()).index(opcode)]
        except ValueError:
